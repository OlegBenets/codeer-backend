...
class IsReviewerOrAdmin(permissions.BasePermission):
    """
    Only the reviewer or admin can update or delete a review.
    """

    def has_object_permission(self, request, view, obj):
        """
        Object-level permission check for review update/delete.
        Allows safe methods for all users.
        Allows modification only if the user is the reviewer or an admin.

        params:
            request: The HTTP request object.
            view: The view that is being accessed.
            obj: The review object to check permissions against.
        returns:
            bool: True if the user has permission, False otherwise.
        """
        if request.method in permissions.SAFE_METHODS:
